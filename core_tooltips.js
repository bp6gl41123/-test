...
// 3. 專屬收錄口袋邏輯
    ...
    return `<div class="pick-tooltip-container"><span class="pick-icon" onclick="event.stopPropagation(); event.preventDefault(); if(typeof window.tooltipGateTrigger === 'function' && window.tooltipGateTrigger()) { event.stopPropagation(); return; } window.toggleMobileTooltip(this);" title="點擊查看今日推薦">💬</span><div class="pick-tooltip"><div class="pick-content">${finalContent}</div><div class="pocket-btn-wrapper"><button class="${btnClass}" onclick="event.stopPropagation(); window.toggleUserPocket('${name}', this, '${sportKey}')">${btnText}</button></div></div></div>`;
};
